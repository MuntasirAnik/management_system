...
namespace Stock.Repository
{
    class ProductRepository
    {
        ...
        public List<Product> Search(string item)
        {
            ...
            //Command
            string commandString = $"select prod.ID, prod.CategoryID,prod.Code,prod.Name,prod.ReOrderLevel,prod.Description, cat.Name AS Category FROM Products AS prod LEFT JOIN Categories AS cat ON cat.ID = prod.CategoryID WHERE (prod.Name LIKE '{item}%') OR (prod.Code LIKE '{item}%')";
            SqlCommand sqlCommand = new SqlCommand(commandString, sqlConnection);
            //open
            sqlConnection.Open();
            SqlDataReader sqlDataReader = sqlCommand.ExecuteReader();
